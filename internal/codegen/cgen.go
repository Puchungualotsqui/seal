...
func (g *Generator) Generate(file *ast.File) string {
	...
	g.resolveRequestedInterfaceImpls()
...
func (g *Generator) implTemplateFromDecl(d *ast.ImplDecl) *ImplTemplate {
	if d == nil || d.Interface == nil || d.Target == nil {
		return nil
	}
	...
	return &ImplTemplate{
		...
		Target:        d.Target,
		Entries:       entries,
		UsingPath:     append([]ast.Ident(nil), d.UsingPath...),
	}
}
...
func (g *Generator) callReturnTypes(expr ast.Expr) []CType {
	...
	if id, ok := call.Callee.(*ast.IdentExpr); ok {
		...
		if len(call.Args) > 0 {
			firstType := g.inferExprType(call.Args[0], nil)

			if g.isInterfaceCType(firstType) {
				if instance, req, ok := g.lookupInterfaceRequirement(
					firstType,
					id.Name.Name,
				); ok {
					return g.interfaceRequirementResultTypes(instance, req)
				}
			}
		}
		...
		if id, ok := e.Callee.(*ast.IdentExpr); ok && len(e.Args) > 0 {
			...
			if g.isInterfaceCType(firstType) {
				if instance, req, ok := g.lookupInterfaceRequirement(
					...
					return g.interfaceRequirementReturnType(instance, req)
				}
			}
		}
...
func (g *Generator) resolveRequestedInterfaceImpls() {
	// First discover every implementation whose interface pattern matches
	// a concrete interface instance.
	for {
		changed := false
		...
		for _, key := range keys {
			...
			for _, template := range g.implTemplates {
				...
				resolved := g.resolveImplTemplateForInterface(
					template,
					instance,
				)
				if resolved == nil {
				...
				if _, exists := g.resolvedImpls[resolved.Key]; exists {
					continue
				}
	...
	// Delegated implementations can only be connected after all ordinary
	// and delegated implementation instances have been discovered.
...
func (g *Generator) resolveImplTemplateForInterface(
	...
	target := g.cTypeFromAstInContext(targetAst)
...
func (g *Generator) matchInterfaceTemplate(
	...
	case *ast.NamedType:
		...
		if len(p.Parts) == 1 {
			return p.Parts[0].Name == instance.BaseName &&
				len(instance.Args) == 0
		}
	...
	case *ast.GenericType:
		...
		if pkgName, _, ok := packageTypeNameFromAst(p.Base); ok {
			if pkgName != instance.PackageName {
				return false
			}
		}
...
func (g *Generator) emitInlineInterfaceWrapperBody(
	...
	if len(entry.Task.Params) > 0 {
		...
		firstType := g.cTypeFromAstWithGenericArgs(
			first.Type,
			info.Subst,
		)
	...
	if pkgName, typeName, ok := packageTypeNameFromAst(typ.Base); ok {
		if pkg := g.packages[pkgName]; pkg != nil {
			...
			if decl := pkg.Structs[typeName]; decl != nil &&
				...
				name := g.registerImportedGenericStructInstance(
					pkgName,
					typeName,
					decl,
					args,
